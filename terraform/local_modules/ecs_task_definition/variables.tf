...
variable "log_configuration" {
  ...
  validation {
    condition     = var.log_configuration != null ? can(regex("^(/[a-zA-Z0-9-]*){4,}$", var.log_configuration.options.awslogs-group)) : true
...
variable "cloudwatch_logs_retention_in_days" {
  type        = number
  description = "Number in days that CloudWatch will keep logs"
  default     = 3
  validation {
    condition     = contains([0, 1, 3, 5, 7, 14, 30, 60, 90, 120, 150, 180, 365, 400, 545, 731, 1827, 3653], var.cloudwatch_logs_retention_in_days)
    error_message = "The cloudwatch_logs_retention_in_days must one of [0, 1, 3, 5, 7, 14, 30, 60, 90, 120, 150, 180, 365, 400, 545, 731, 1827, 3653]."
  }
}
...
variable "enable_ecr_access" {
  description = "Whether to attach the `AmazonEC2ContainerServiceforEC2Role` to the ECS Task Execution role"
  type        = bool
  default     = false
}

variable "requires_compatibilities" {
  description = "The list of compatibilities for the service"
  type        = list(string)
  default     = ["FARGATE"]
}

variable "volume" {
  description = "The volume for the service"
  type        = list(any)
  default     = []
}
...
variable "enable_ecs_exec" {
  description = "Whether to attach the SSM permissions to the ECS Task Execution role"
  type        = bool
  default     = false
}
